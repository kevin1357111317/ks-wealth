...
test('新增財務項目選台股就能記交易，而且不會重複記帳', { skip }, async t => {
  ...
  await t.test('第一筆交易寫進私帳，資產列由觸發器產生', async () => {
  ...
  await t.test('交易區留白就只改標的本身', async () => {
    ...
    assert.equal(data.klfan_stocks[0].symbol, 'TPE:2317', '改代號要寫回私帳');
    assert.equal(data.klfan_stocks[0].display, '鴻海', '名稱跟著證交所走');
  });

  await t.test('私帳卡片就地展開，不跳頁', async () => {
  ...
  await t.test('刪除要連私帳一起刪，否則觸發器會把它重建回來', async () => {
    await openCard();
  ...
  await t.test('其他資產類別完全不受影響', async () => {
    ...
    assert.equal(data.klfan_stocks.length, 0, '現金不該碰私帳');
  });
  ...
  await t.test('夫妻持有同一檔股票要分開私帳，不互相改歸屬', async () => {
    ...
    const data = await db();
    const tsmc = data.klfan_stocks.filter(row => String(row.symbol).endsWith('2330'));
    assert.equal(tsmc.length, 2, '夫妻各自持有同一檔時要有兩個獨立私帳標的');
  ...
  await t.test('進私帳從最上面開始，返回回到原本停的位置', async () => {
    await page.setViewportSize({ width: 390, height: 500 });
    ...
    await page.click('[data-open-portfolio]');
    await page.waitForSelector('[data-portfolio-market]');
    assert.equal(await page.evaluate(() => window.scrollY), 0, '進私帳要從最上面開始，不是接著原本的位置顯示');
  ...
  await t.test('分析入口排在資產／負債切換的上面', async () => {
  ...
  await t.test('美金分析可以記買賣，數字跟著動，也退得回來', async () => {
    ...
    assert.equal(await page.locator('[data-portfolio-market]').count(), 0, '進的是美金分析，不是股票私帳');
  ...
  await t.test('老婆頁也有各項分析入口，而且看到的是自己的部位', async () => {
    await page.click('[data-tab="wife"]');
    await page.waitForSelector('.fab');
    ...
    assert.equal(await page.locator('[data-open-loans]').count(), 1, '老婆頁也要有貸款分析');
    assert.equal(await page.locator('[data-open-insurance]').count(), 1, '老婆頁也要有保險分析');
