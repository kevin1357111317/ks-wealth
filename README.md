...
證據是每個月不管幾天都固定扣 43,219 —— 23,790,000 × 2.18% ÷ 12 = 43,218.5，四捨五入剛好
對上；actual/365 的話每個月會不一樣（將來、富邦就是那樣）。

#### 排程是從銀行的「本息攤還表」反推的

攤還表上寬限期只繳息一路到**第 37 期**，第 38 期才開始還本金 —— 照「寬限期間
2024-03-01~2027-03-01」推的 35 期是錯的，實際多繳兩期利息。

反推出來的規則：月付 97,207.69 對應**324 期**的年金（323 期是 97,428、325 期是 96,988，
都對不上），利息 = 餘額 × 2.18% ÷ 12，**餘額不四捨五入往下帶**。用這組規則重算，攤還表上
抽查的 12 列有 11 列分毫不差（只有第 114 期本金差 1 元，精確值 61,974.64 卡在進位邊界），
而且最後一期餘額剛好歸零 —— 這三件事同時成立才敢說算法對了。

第 38 期起 324 期會排到 2054-04-05，比「借款期間 2054-03-01」多一個月；銀行的月付金額就是
這樣算出來的，攤還表表尾還沒看到，之後要再對一次。

拆成兩半各自四捨五入會有誤差，但有界：中段最多差 104 元，到期時回到 0。

### 潤隆房貸對不起來的那 1,878 元

用單一利率 2.18% 從撥款日重播 36 期，餘額是 7,414,464，銀行是 7,412,586，**多 1,878 元**
（用 actual/365 是多 2,342）。方向是對的 —— 早期利率比較低就會多還本金、餘額比較低 ——
但**幅度對不上央行升息**：2024-03-21 那次 +0.125%，假設之前是 2.055%，重播會變成少 3,400 到
8,557，反而過頭。

換算成平均利率，這個差距只值 **0.008 個百分點**（2.172% vs 2.18%），比一次升息小一個數量級。
前 2 期 2.048%、前 6 期 2.136%、前 12 期 2.158% 都能兜出同一個答案，資料不夠沒辦法分辨是
前幾期優惠利率、指數延遲、還是月付金額本身就不是 30,287。要修的話一樣需要中信 App 的
攤還表或繳息明細。

誤差不會滾大：`apply_due_loan_payments()` 每期都拿當下的實際餘額重算利息，不是從舊的推算值
往下疊，所以 1,878 就停在那裡。
...
`pinLoanCard()` 記下被點那張的視窗位置，換完卡片再把捲動量補回去；`loan-ui-fix.js` 會在
後面幾個 frame 再搬一次 DOM，所以連續三個 frame 各重量一次，每次都重新量而不是套用
同一個差值。
