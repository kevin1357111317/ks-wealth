...
保險分析畫面先依保險公司整合，再在公司區塊內保留逐張保單明細；公司分組不改變有效主約數、
保費彙總或現金價值計算，也不會把同公司的保額相加成單一保障。

## 夫妻健康報告

底部導覽的順序是家庭、老公、老婆、健康。登入後不是落在家庭總覽，而是落在自己的資產頁：
`household_members` 目前沒有 user 對 `owner_scope` 的欄位，只有 `role`，所以用「建立家庭的是老公、
用邀請碼加入的是老婆」來推（`memberOwnerScope()`）。之後要支援反過來的家庭，就把 `owner_scope`
存進 `household_members`，只要改那支函式。

第四個分頁（愛心）進入獨立健康模組，可切換老公／老婆與年度。V3.28.0 之前它是家庭頁
最上面那張大卡片，佔掉整個第一屏才進得去；它跟三個資產分頁是同一層的目的地，所以改成分頁。
走 `tab` 而不是 `analysisScreen`：底部導覽才會把它標成 on，也不會在歷史多推一筆。

畫面最上方固定用最新年度整理具體行動，年度切換則用來查看當年重點；趨勢只顯示備孕或健康
決策需要優先關注的指標，不把整張檢驗單原樣搬進 App。

健康管理評分存成年度指標，只用來整理已提供資料的管理優先順序，不是疾病風險量表。重要趨勢
會優先挑最新異常且至少有兩個年度的數值，再補上各人需要長期維持的核心指標；不同年度缺少
某項檢驗時只略過該年度，不會讓整個健康頁失效。圖表同時畫出參考下限與上限，不能把偏低項目
錯拿上限當警戒線。

健康頁預設先顯示「夫妻總覽」：並排呈現兩人的總分、六大評分類別、共同檢驗項目與各自優先事項；
個人頁仍可查看各年度趨勢。比較欄位只呈現各自正常／異常狀態，不用單純數字高低判定誰比較健康。

### 參考區間統一成國際標準

每年健檢換實驗室，同一個指標的參考區間就會跟著換（飯前血糖 70–99 vs 70–100、MCV 80–95 vs
80–100、ALT 5–35 vs 10–40），夫妻並排時圖上會冒出好幾條意義不同的參考線，看起來像兩套標準。
`health-reference.js` 因此把常用指標統一成國際通用的成人標準，`buildHealthModel` 讀資料時就
套上去，狀態（正常／偏低／偏高）也依同一組區間重算 —— 不然會出現「參考範圍內」卻標紅字的矛盾。

醫學上本來就男女有別的項目才分開列，其餘男女共用一組數字：血色素、血球容積比、紅血球、
ALT、AST、γ-GT、ALP、肌酸酐、尿酸、HDL、腰圍，以及只有男性適用的精液分析。來源寫在
`health-reference.js` 檔頭：CBC 用成人常用參考區間、血糖與糖化血色素用 ADA、血脂用
NCEP ATP III／AHA、ALT 用 AASLD 2017 健康人上限、eGFR 用 KDIGO（<60 為腎功能下降）、
精液用 WHO 2021 第 6 版下限值、體位用 WHO BMI 與 IDF 亞洲腰圍切點。

沒收錄的指標保留原報告數值：腫瘤標記（CEA、CA19-9、PSA）、肝炎與感染篩檢、胰島素、HOMA-IR、
G6PD、體脂率這些切點是儀器與方法決定的，硬套別家的標準會把正常判成異常或反過來。AFP 是唯一
的例外：兩人的報告一個寫 0–7、一個寫 <9，圖上會多出兩條意義相同的參考線，所以統一成常見成人
切點 0–10 —— 老公 5.73 仍是正常、老婆 12 仍是偏高，判定沒有變。
統一之後全部資料只有兩筆判定改變，而且都是原本被實驗室較嚴的區間判成偏低的：老公 2026 年
血球容積比 41.9%（國際下限 41，該年實驗室抓 42）與 2025 年 ANC 1.76（國際下限 1.5，該年
實驗室抓 1.85）回到正常。其餘維持原判定：老公白血球 3.58–3.93 偏低、總／直接膽紅素偏高、
2024 年肌酸酐 0.68 偏低；老婆 MCV、MCH、MCHC、RDW、紅血球、2026 年血色素 11.9 與總膽固醇
209／218 仍然是異常。

趨勢圖兩個人用藍（鎧麟）／橘紅（佳軒）兩個色相差最大的顏色，而且點的形狀不同（圓點對菱形），
色弱或黑白列印時一樣分得出來；參考線同色，男女共用同一組區間時只畫一條共用線加底色區間。
卡片不重複固定顯示兩人的最新數值；直接點選或左右滑動圖上的年份，提示框會同時列出兩人該年的值，
缺測的一方顯示「—」。這樣手機畫面不會被摘要框擠滿，也能自行逐年核對。

目前老公有 2023／2024／2025／2026、老婆有 2025／2026 的年度資料：老公優先顯示白血球、ANC、
膽紅素與脊椎發現，老婆優先顯示小球性紅血球型態、AFP、尿液複查與備孕時程。後續新增報告時仍
先人工核對，只把需要追蹤、會影響決策或可形成趨勢的內容放到首頁；其他正常數值保留在資料庫供
年度比較。

### 什麼會顯示、什麼只留在資料庫

一份健檢報告會全部匯入 `health_metrics`，但畫面只呈現兩種東西：趨勢圖挑「最新年度異常且至少
兩個年度有數值」的指標，個人頁的分類卡片挑 `buildHealthDomains` 明列的 key。沒被挑到的就只
存在資料庫裡，不會出現在 App —— 這是刻意的，不是漏做。

所以像身體組成細項（除脂體重、骨骼肌肉量、細胞內外水、基礎代謝率、阻抗值、四肢肌肉量與水腫
指數）、白血球分類百分比、眼科視力與眼壓、CYFRA 21-1 這些都只進資料庫。眼科、白血球分類這種
真的沒事的寫 `normal`，機器推算值（基礎代謝率、代謝年齡、阻抗值、內臟脂肪重量）寫 `info`；
身體組成的偏低項目仍照 X-SCAN 判定寫 `low`，不美化數字。

要留意的是「不顯示」目前是靠**最新年度沒有這個欄位**達成的 —— 趨勢候選只從最新年度的指標裡
挑。哪天新報告也量了體脂機，這些 `low` 就會變成趨勢候選並自己冒上首頁；到時候再決定是要讓它
顯示（多半該顯示）或把狀態改成 `info`，不要靠運氣。
...
健康資料載入失敗時不得拖垮財務首頁；健康模組可以暫時顯示空資料，資產、貸款與行情仍要正常。
...
- 台股那一欄收代號也收中文名（`2330` 或「台積電」都可以），打完會顯示查到的對照；
  查不到就照輸入的存
- **同一個人**同一檔再買一次會併回原本的標的（用 `owner_scope + market + 裸代號` 比對，
  前綴不算數）—— 出清過的舊紀錄才不會跟新買的分成兩筆各自為政；夫妻若都持有同一檔，
  必須各自保留不同的 `klfan_stocks.key`，交易與 `financial_items` 不可互相混用
...
報價代號在 Kevin 私帳裡一律帶交易所前綴（`TPE:2330`、`NASDAQ:QQQ`），`sync_klfan_financial_item()`
就是靠這個前綴把裸代號切出來寫進 `financial_items.symbol`。表單允許只打 `2330`，會自動補上
並顯示補完的結果。

Kevin 私帳連動的股票出清後，`financial_items` 那一列是被更新成 0 股 0 元、**不是刪掉** —— 唯一索引建在
`portfolio_stock_key` 上，再買回來要能接回同一列。所以 `getItemsForScope()` 在顯示層把 0 股的
Kevin 私帳部位濾掉；原始的 `items` 陣列不動，`syncPortfolioFinancialItem()` 還要靠它找到那一列。
手動建立的 0 元項目不受影響。
...
黃金分析把即時部位與 KLFAN「黃金」工作表的成本 Kevin 私帳分開保存：`financial_items` 繼續負責目前重量、
即時金價與台幣市值，`gold_transactions` 只保存每筆買進日期、成本、重量與收藏工錢。年化報酬率用每筆
實際投入日期重算 XIRR，不沿用試算表可能過期的快取結果。

成本 Kevin 私帳重量若小於資產頁重量，畫面會明確顯示缺少成本的克數；損益與 XIRR 只計算已有成本且標示
「計入年化」的部位，不把差額或送人的黃金當成零成本獲利。

PAMP 星座的 NT$52,696 是完整買進支出，其中含工錢 NT$23,080。工錢已包含在投入成本中；
依使用者指定的資產估值口徑，目前價值以 6g 即時金價再加回 NT$23,080 工錢。

### 送出去的黃金只留紀錄

送父母那兩筆各 2.5g **不是自己的部位了**，所以三個地方都要一致：不算年化、不算目前持有、
資產頁的重量也要扣掉。這是 `still_held = false`（`include_in_performance = false` 只擋年化，
重量還是留在資產頁上，兩邊會對不起來）。

`gold-core.js` 把「送出去的一定不計年化」寫死在 `normalizeGoldTransaction()` 裡
（`include_in_performance !== false && still_held !== false`），兩個旗標不容許互相矛盾 ——
東西都送人了，期末沒有對應的價值可以配對，XIRR 會把它當成有去無回的虧損。

資產頁的重量跟著從 204.3105 g 改成 **199.3105 g**（每公克單價不變，市值照比例縮成
882,213）。重量核對只比對還持有的那幾筆。家庭淨資產因此少約 NT$ 22,100 —— 那本來就不該
算進來。

### 摘要不重複講同一個數字

黃金摘要本來有六格，前兩格是「目前黃金部位／目前台幣市值」。送出去的克數扣掉之後，
部位的克數就跟「投入成本」那一格底下標示納入年化的克數是同一個數字，市值也只跟
「目前價值」差一筆工錢 —— 兩格都是重複的，拿掉。現在是四格，全部都在講年化那一組。

「重量已核對」那一塊同樣只在**對不起來**的時候才畫：對得起來時它只是把上面的克數再寫
一次。缺少買進成本的警告一定要留著，不然差額會靜靜地不見，`tests/gold-analysis.test.mjs`
兩邊都有守。
...
新增股票只有一個入口：「新增財務項目」表單。 Kevin 私帳頁不放第二個新增入口，也沒有返回按鈕。

「股票投資」是狀態切換不是換頁，返回手勢預設不會有反應，所以進去時 `window.history.pushState()`
推一筆、`popstate` 再把畫面收回來（`app-v3.js` 有個模組層的 `history` 變數存淨資產歷史，會遮蔽
`window.history`，一定要寫全）。底部導覽切分頁也走同一條路 —— `render()` 在 Kevin 私帳頁會直接回傳
Kevin 私帳畫面，不先收掉會被困住。頁面上沒有返回按鈕。

「股票投資」清單的卡片是**就地展開**的，不跳頁：展開後在原位往下長出已實現與未實現損益、
累計股息、目前股價、投資期間、持有股數，以及完整交易紀錄。收合的卡片只顯示市值、累計損益與
XIRR 年化報酬 —— 每個數字只出現在其中一邊，不重複。
一次只開一檔，再點一次收起來。

**投資期間看的是「錢投在裡面多久」，不是「買了多久以前」**：還握著的部位算到今天，已經出清
的算到最後一次買賣那天。金益鼎 2023-08-04 買、2023-08-09 賣，實際只有 5 天；舊版一律算到
今天，2026 年打開會顯示 3.12 年，看起來像抱了三年才賠。賣光之後才入帳的股息不算進期間
（那只是尾款），但 XIRR 仍然吃它的日期。不滿一年用天顯示 —— 當沖與短線部位用年當單位會
全部擠成「0.00 年」。年化報酬率本來就是用每筆現金流的真實日期算的，不受這個欄位影響。
...
美股分析頁固定使用單一 USD 口徑，避免同一卡片混入美元、台幣與匯率影響：
...
- 清單可依市值、累計損益、XIRR 年化做高低排序；沒有可計算數值的標的一律排最後

這是呈現口徑的修正，不改交易資料、資產市值或其他頁面的計算定義。

### 投資組合 vs 大盤趨勢

股票分析摘要下方使用「累積 TWR 指數，起始＝100」，不拿市值或 XIRR 畫折線：每日股票市值先扣除
...
`portfolio-performance` Edge Function 直接從完整交易 Kevin 私帳重建每日持股，配合 Yahoo Chart 的歷史
...
同一區塊另列三組不混用的口徑：累積 TWR 是圖表期間的總報酬；期間至少 365 天時，年化 TWR 以
`(1 + 累積 TWR) ^ (365 / 實際日數) - 1` 計算，投資組合與 Benchmark 共用完全相同的起訖日，差距
以百分點（pp）呈現；未滿一年一律顯示「—」。期間 XIRR 則把第一天市值視為期初投入，再套用期間內
真實的每筆投入／提款金額與日期及期末市值。Benchmark XIRR 使用完全相同的資金時點，只把資金依
當日含息 Benchmark 指數換算為單位；全部頁的混合 Benchmark 仍沿用前一日實際台美股權重，不退化成
固定 50/50。上方摘要的 XIRR 仍是完整交易 Kevin 私帳的實際資金年化報酬，TWR、XIRR 與累計損益率不互換。

Yahoo 的 `close` 已回溯調整拆股、但不調整現金股息；持股市值繼續使用 `close`，Kevin 私帳股息當提款
...
倍率同時支援小於 1 的反向調整：長榮 2022 現金減資後，Yahoo 會把 2021 歷史價格回溯調高，而 Kevin 私帳
仍保存減資前股數；舊邏輯因此在 2021-11-08 產生約 +37.4% 的假尖峰。現在以 0.4 倍重建舊股數，並
移除原本直接略過極端單日報酬的遮罩，之後異常資料會在測試與圖表中暴露而不是被靜默隱藏。

每日 TWR 扣掉的當日金流，用「當日收盤價 × 成交股數」計價，不用 Kevin 私帳的成交金額。市值本來就是收盤價
算出來的，拿成交金額去扣，等於把成交價與收盤價的價差（手續費、盤中成交價位）當成當天的報酬；部位
還很小的時候加碼，這個價差除以前一日市值就會炸成幾十個百分點的假單日報酬：

| 日期 | 事件 | 前一日市值 | 成交價 vs 當日收盤 | 舊的單日報酬 | 修正後 |
| --- | --- | --- | --- | --- | --- |
| 2020-07-22 | 買 1 張台積電 381,336 | 10,956 | 381.34 vs 384.00 | **+24.57%** | +0.25% |
| 2020-08-18 | 買 1 張台積電 440,388 | 23,021 | 440.39 vs 433.00 | **−32.66%** | −0.57% |
| 2020-11-17 | 買 1 張台積電 502,443 | 63,394 | 502.44 vs 485.50 | **−26.02%** | +0.71% |
| 2019-12-19 | 買 1 張 0050 95,186 | 23,372 | 95.19 vs 97.65 | **+9.98%** | −0.56% |

這四天全是 2020-07-17 賣掉 0050 主要部位之後、帳上只剩一萬多元時的加碼，方向與價差符號一致，不是行情
也不是除權息或拆股。這一項單獨讓全期（2019-10-23 ~ 2026-09-17）累積 TWR 從 206.18% 回到 471.24%、
年化 TWR 從 17.59% 回到 28.70%；XIRR 27.09% 完全不變 —— XIRR 是資金加權，本來就該用 Kevin 私帳的真實現金金額，所以
`transactionFlows` 維持原樣，只有 TWR 那條線改用收盤價計價的金流（`buildHistoricalSnapshots` 會把它
掛在每日快照的 `flow` 欄位上）。代價是成交價與收盤價的價差不再計入 TWR，但那段本來就被日收盤口徑的
TWR 攤成雜訊，而且它仍完整反映在 XIRR 與累計損益裡。剩下的單日極值（2025-04-09 +10.35%、2020-07-27
+9.77%、2020-03-20 +7.95%、2025-04-03 −7.52%）當天都沒有金流，是真的行情。

圖表右上角的期間與 X 軸刻度都帶年份（`YYYY/M/D`）：只有 `M/D` 時「9/17 – 9/17」看起來像同一天。X 軸
只在第一個刻度與跨年處補年份，同年的刻度維持 `M/D`，380px 下三個刻度都放完整日期也不會重疊或換行。
年化比較區塊在期間未滿一年時標「期間未滿一年，不年化」（`annualizeReturn` 本來就回 `null`，畫面顯示
「—」，沒有把短期報酬放大年化過）；期間約一年時年化必然等於累積，比較實際值：差距小於 0.005pp（連
兩位小數都一樣）標「期間為一年，年化＝累積」，週年日碰到假日使期間多一兩天時標「期間約一年，
年化≈累積」。說明數字一樣不是壞掉，比整塊藏起來動得更少。

同一批還修掉四個會讓 TWR 悄悄失真的口徑問題：

- **股息對齊除息日。** Kevin 私帳記的是入帳日，價格卻是在除息日掉的，兩者常差 20~55 天。算在入帳日等於
  除息日先跌一次、入帳日再漲一次；同一期間內大致抵銷，跨期間邊界就會少算或多算。改成用 Yahoo
  `events.dividends` 的除息日（差超過 120 天就不算同一次配息，退回 Kevin 私帳日期）。XIRR 用的
  `transactionFlows` 不動：資金加權本來就該用真實入帳時點。
- **不完整日的金流不再消失。** 只要有一檔持股當天抓不到價格，整天就不產生快照；原本那天的買賣金流
  會跟著蒸發，之後每一天的 TWR 都永久偏掉。現在分兩層補：還沒有任何報價的買賣，等它第一次有價格
  時才計入（跟它進入市值的時點一致）；已經有價格但當天沒產生快照的金流，累到下一個算得出市值的
  日子。`buildPerformanceSeries` 也一樣，這個 market 當天市值為 0 的日子會把金流寄放到下一天。
  目前資料沒踩到（缺價的標的從頭缺到尾），但這是會靜默算錯的定時炸彈。
- **股數倍率改用真的拆股事件收斂。** 原本純粹用「成交單價 ÷ 當日收盤價」在十五個候選倍率裡猜，
  容忍度 18%。現在若 Yahoo `events.splits` 有資料，候選只留「1（Kevin 私帳已換算）」與「成交日之後的累計
  倍率（Kevin 私帳是成交當時股數）」兩個；沒有資料才退回原本的猜測清單。NVDA 2024-06-10 的 10:1 因此
  不再有誤判空間。

### 台灣 ETF 的分割要自己查表

Yahoo 的 chart API **不回報台灣 ETF 的受益權單位分割** —— 它把歷史價格調整了，卻不給 `events.splits`。
0050 的 2019 收盤直接從 87 元變成 22 元，`splits` 欄位卻整個不存在（2026-09-19 直接打 API 確認）。
美股與台灣個股都有回報：NVDA、TSLA、SOXX、MUU、長榮 2022 的現金減資（`400:1000`）都查得到。

所以 `TW_ETF_SPLITS` 這張表要自己維護，日期用「分割後恢復交易日」，也就是第一個用新單位成交的日子：

| 標的 | 恢復交易日 | 倍率 | 對照 |
|---|---|---|---|
| 0050 | 2025-06-18 | 4 | 188.65 → 47.16 |
| 00631L | 2026-03-31 | 22 | 443.15 → 20.14 |

**00631L 是 1 拆 22，不是 20。** 舊的猜測清單裡根本沒有 22 這個候選，只能猜成最接近的 20，等於
那段期間的持股一直少算 9%。這就是「用猜的」撐不住的地方 —— 新增一檔時去查投信公告，把價格對照
一起寫進註解，之後才對得起來。

- **同一段期間的倍率只決定一次。** Kevin 私帳的單位在同一個拆股期間只會有一種慣例（要嘛全是成交當時
  股數，要嘛全部已經換算）。逐筆各自判斷會被一筆壞資料帶走：0050 Kevin 私帳 2023-01-30 那筆金額只記了
  一半（60 股 3,620 元，當天實際約 120.7 元／股），單價比值算出來剛好是 2，自己跑出一個不存在的
  倍率，整檔持股就少 120 股。改成同一段期間多數決之後，六檔現有持股的股數全部對得上資料庫。
- **美股沒有拆股事件就不要猜。** Yahoo 對美股的回報是可靠的，沒回報就是真的沒拆過。原本會把
  對不上市價的零股交易（手續費、拆單）當成拆股證據 —— QQQ／SOXX／VOO 在 2026-05-13 的零股賣出
  被猜成 1/3、0.4、0.25，TSLA 2025-11-13 被猜成 0.5，憑空多出 1.18／0.43／0.21 股。
- **期間基準點取起算日當天或之前最後一個交易日。** 今年以來因此從去年最後一個交易日的收盤起算
  （標準 YTD 口徑，原本是今年第一個交易日、等於少算了第一天）；近一年的週年日碰到假日時也不會
  只剩 364 天、讓年化整排顯示「—」，週年日本身有交易時仍維持剛好 365 天。

另外「全部」期間兩千多個交易日抽成 280 點，平均每七天才留一點，2025-04-09 那種單日 +10% 會整根被
跳過。`downsampleSeries` 均勻抽樣之後，額外把單日振幅 2% 以上、最大的 12 天連同前一天補回來
（上限 +24 點）。只影響圖形；累積 TWR、年化與 XIRR 一律用完整序列計算。

四項合計之後的數字（`market=all`、mixed benchmark，today 取 2026-09-17）：

| 期間 | 累積 TWR | 年化 TWR | XIRR | 期間長度 |
| --- | --- | --- | --- | --- |
| 今年以來 | 31.53% → 34.39% | — | 46.02% → 47.40% | 258 → 260 天 |
| 近一年 | 49.00% → 50.41% | 49.00% → 50.41% | 47.41%（不變） | 365 天 |
| 全部 | 206.18% → 465.84% | 17.59% → 28.52% | 27.09%（不變） | 2,521 天 |

拆開看：金流改用收盤價計價讓全部期間 206.18% → 471.24%，除息日對齊再把它修到 465.84%（年化
28.70% → 28.52%）；期間基準點只動到今年以來（32.96% → 34.51%，XIRR 46.02% → 47.40%）。Benchmark
那條線完全沒變 —— 它只吃價格、匯率與前一日台美股權重，不讀任何金流。
...
補正**補得回來的上限是目前的捲動量**。收起上面那張少掉的高度要靠往回捲來抵，而捲動量不能
是負的：捲動量比那個高度差小的時候，捲到 0 就沒得退，剩下的差距是物理上補不回來的
（例如剛把下面那張捲進畫面就點它，捲動量可能只有 950、高度差卻有 1034，卡片就會往上跑 84px）。
`pinLoanCard()` 已經把能用的捲動量用到見底，這是最好的結果。

測試挑起始捲動位置時**要從實際量到的高度差去推，不能寫死**。展開後的卡片高度跟字體換行
有關，CI 用 `npx playwright install` 抓的 Chromium 比本機的 `/opt/pw-browsers/chromium-1194`
高了約 191px，寫死的 1200 在那個版本上比高度差還少 25px，測到的就會是「捲不回去」而不是補正本身。

`.loanList` 要 **`overflow-anchor:none`**。Chrome 的捲動錨定（scroll anchoring）會在上面的
內容高度變動時自己調整 `scrollY`，好讓視窗裡看得到的東西不要位移 —— 那跟 `pinLoanCard()`
是同一件工作的兩套機制，開著就會互搶。實測：測試把捲動量設成 1234、接著點卡片，
`pinLoanCard()` 第一次量到的 `scrollY` 是 **1565**，中間那 331px 沒有任何 `scrollTo`、
也沒有 `scroll` 事件，就是錨定自己調的。補正因此是在追一個會自己跑的目標，最後停在哪裡
跟當下的 layout 時機有關；`overflow-anchor:none` 之後第一次量到的就是 1234。

2026-09-18 CI 為此紅過幾次（被點的卡片停在 670.11，`scrollY` 停在 0，但當下 `maxScroll`
有 1835、只需要捲到 200.81 —— 不是捲不動）。查的過程走過兩次錯路，都記在這裡免得再走一次：
先猜「CI 太忙、rAF 來不及補正」，把測試的等待改成「連兩個 frame 沒再動、最多 5 秒」之後
CI 還是紅，證明不是等太短；再猜「捲動量不夠、捲到 0 就沒得退」，本機把捲動量改成 950
的確能重現同一個數字，但那是另一條路徑碰巧停在同一點 —— CI 那次 `maxScroll` 明明夠。
真正分辨出來的方法是**把每一次 `scrollTo` 連呼叫點、以及每一個 `scroll` 事件都記下來**，
一看就知道那 331px 不是程式捲的。
...
| 美股（快車道） | 15 秒 | Finnhub | 9 檔 ÷ 15 秒 = 36 次／分，上限 60 | **不寫** |
| 美股（權威值） | 60 秒 | Finnhub（備援 Twelve Data） | 同上 | 寫 |
| 匯率／黃金 | 60 秒 | Twelve Data | 一輪 2 credits，上限每分鐘 8 | 寫 |

美股快車道（`scope='us'`）跟台股那條是同一個模式：只拿價格、不寫資料庫，前端直接套用到畫面。
**15 秒不是保守值，是算出來的**：Finnhub 免費每分鐘 60 次，9 檔標的 10 秒一輪就是 54 次／分，
太貼邊；15 秒是 36 次／分，留得下重試與手動更新的空間。

快車道**不讀 `klfan_quotes` 的快取** —— 快取最舊可以到 14 秒前，那正好是這條路想擺脫的延遲。
...
碰了的話 `financial_items` 的 realtime 訂閱會被自己觸發，每 5 秒重載整本 Kevin 私帳（一千多筆交易）
...
開 App 的首屏**不等待 Kevin 私帳**。資產列的股數與市值是 `sync_klfan_financial_item()` 觸發器算好存在
`financial_items` 的；首屏完成後才用 `requestIdleCallback`（不支援時退回短延遲 timer）在背景預載
92 KB Kevin 私帳，讓股票分析一點就能進去：
...
`tests/awake-autorefresh.test.mjs` 守住「首屏不等待 Kevin 私帳、背景最多預載一次、切到個人頁與行情更新
...
所以模型走 `portfolioModelFor(ownerScope)`：用 `portfolioRevision`（Kevin 私帳重載、報價套用、
