...
月付 21,609.5），兩邊加起來才是銀行 App 上的數字。負債列本來就已經是分開的兩列，這次只補
分析用的主檔與排程。月付留小數是為了讓兩半相加剛好等於 43,219，卡片上顯示會四捨五入成
21,610。

`interest_day_count` 記各家銀行的算法：玉山這筆是**年利率 ÷ 12**，其他信貸是 **actual/365**。
證據是每個月不管幾天都固定扣 43,219 —— 23,790,000 × 2.18% ÷ 12 = 43,218.5，四捨五入剛好
對上；actual/365 的話每個月會不一樣（將來、富邦就是那樣）。兩種算法差幾百塊，寬限期內因為
本金不動所以看不出來，2027 年出寬限期之後才會咬人。

寬限期 2024-03-01 ~ 2027-03-01 只繳息，跟 App 上「本金已償還 0%」對得上。**2027-03-05 起
325 期、每半份 48,494 是照 2.18% 自己試算的，不是銀行給的數字** —— 玉山 App 裡有「本息攤還
表」，拿到之後要用實際數字覆蓋。30 年的浮動利率本來就會變。

進貸款分析預設停在「這個人真的有貸款」的那一頁（`firstLoanTypeWithRows()`）。老婆只有房貸，
固定停在信貸的話一進來會看到整頁的 0。
