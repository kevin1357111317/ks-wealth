...
第 38 期起 324 期會排到 2054-04-05，比「借款期間 2054-03-01」多一個月 —— **這是繳款日改過期
造成的，不是資料錯誤**。原本繳款日是每月 1 日，那時候每個邊界都剛好對齊：

| | 期別 | 日期 | 對照 |
| --- | --- | --- | --- |
| 寬限期最後一期 | 第 36 期 | 2027-03-01 | = 寬限期間結束 |
| 全期最後一期 | 第 360 期 | 2054-03-01 | = 借款期間結束 |

也就是原本就是「寬限 36 期 + 攤還 324 期」，而 324 期的年金正好是 97,207.69 —— 反推出來的
期數不是湊的，那就是銀行原始的合約。改成每月 5 日之後切換點順延一期（寬限期變成到第 37
期），324 期攤還跟著排到第 361 期。

寬限期本金不動、利息又是 ÷12 跟天數無關，所以早期那幾期的 `due_date` 記成 5 日而不是 1 日，
不影響任何算出來的數字。
