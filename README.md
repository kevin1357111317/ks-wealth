...
測試挑起始捲動位置時**要從實際量到的高度差去推，不能寫死**。2026-09-18 CI 紅過一次
（被點的卡片從 695.30 跑到 670.11），查出來就是這個：展開後的卡片高度跟字體換行有關，
CI 用 `npx playwright install` 抓的 Chromium 比本機的 `/opt/pw-browsers/chromium-1194`
高了約 191px，寫死的 1200 捲動量在那個版本上比高度差還少 25px，捲到 0 還差 25px。
本機把捲動量改成 950 就能一模一樣地重現（同樣停在 670.109375）。當時先猜「CI 太忙、rAF
來不及補正」是錯的方向 —— 把等待改成「連兩個 frame 沒再動、最多 5 秒」之後 CI 還是紅同一個數字，
那就證明不是等太短，是真的捲不回去。
