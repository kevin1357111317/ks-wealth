...
報價代號在私帳裡一律帶交易所前綴（`TPE:2330`、`NASDAQ:QQQ`），`sync_klfan_financial_item()`
就是靠這個前綴把裸代號切出來寫進 `financial_items.symbol`。表單允許只打 `2330`，會自動補上
並顯示補完的結果。

私帳連動的股票出清後，`financial_items` 那一列是被更新成 0 股 0 元、**不是刪掉** —— 唯一索引建在
`portfolio_stock_key` 上，再買回來要能接回同一列。所以 `getItemsForScope()` 在顯示層把 0 股的
私帳部位濾掉；原始的 `items` 陣列不動，`syncPortfolioFinancialItem()` 還要靠它找到那一列。
手動建立的 0 元項目不受影響。
...
黃金分析把即時部位與 KLFAN「黃金」工作表的成本私帳分開保存：`financial_items` 繼續負責目前重量、
即時金價與台幣市值，`gold_transactions` 只保存每筆買進日期、成本、重量與收藏工錢。年化報酬率用每筆
實際投入日期重算 XIRR，不沿用試算表可能過期的快取結果。

成本私帳重量若小於資產頁重量，畫面會明確顯示缺少成本的克數；損益與 XIRR 只計算已有成本且標示
...
新增股票只有一個入口：「新增財務項目」表單。私帳頁不放第二個新增入口，也沒有返回按鈕。

「股票投資」是狀態切換不是換頁，返回手勢預設不會有反應，所以進去時 `window.history.pushState()`
推一筆、`popstate` 再把畫面收回來（`app-v3.js` 有個模組層的 `history` 變數存淨資產歷史，會遮蔽
`window.history`，一定要寫全）。底部導覽切分頁也走同一條路 —— `render()` 在私帳頁會直接回傳
私帳畫面，不先收掉會被困住。頁面上沒有返回按鈕。
...
`portfolio-performance` Edge Function 直接從完整交易私帳重建每日持股，配合 Yahoo Chart 的歷史
...
倍率同時支援小於 1 的反向調整：長榮 2022 現金減資後，Yahoo 會把 2021 歷史價格回溯調高，而私帳
仍保存減資前股數；舊邏輯因此在 2021-11-08 產生約 +37.4% 的假尖峰。現在以 0.4 倍重建舊股數，並
移除原本直接略過極端單日報酬的遮罩，之後異常資料會在測試與圖表中暴露而不是被靜默隱藏。

每日 TWR 扣掉的當日金流，用「當日收盤價 × 成交股數」計價，不用私帳的成交金額。市值本來就是收盤價
...
年化 TWR 從 17.59% 回到 28.70%；XIRR 27.09% 完全不變 —— XIRR 是資金加權，本來就該用私帳的真實現金金額，所以
...
- **股息對齊除息日。** 私帳記的是入帳日，價格卻是在除息日掉的，兩者常差 20~55 天。算在入帳日等於
  除息日先跌一次、入帳日再漲一次；同一期間內大致抵銷，跨期間邊界就會少算或多算。改成用 Yahoo
  `events.dividends` 的除息日（差超過 120 天就不算同一次配息，退回私帳日期）。XIRR 用的
...
- **股數倍率改用真的拆股事件收斂。** 原本純粹用「成交單價 ÷ 當日收盤價」在十五個候選倍率裡猜，
  容忍度 18%。現在若 Yahoo `events.splits` 有資料，候選只留「1（私帳已換算）」與「成交日之後的累計
  倍率（私帳是成交當時股數）」兩個；沒有資料才退回原本的猜測清單。NVDA 2024-06-10 的 10:1 因此
  不再有誤判空間。
...
- **同一段期間的倍率只決定一次。** 私帳的單位在同一個拆股期間只會有一種慣例（要嘛全是成交當時
  股數，要嘛全部已經換算）。逐筆各自判斷會被一筆壞資料帶走：0050 私帳 2023-01-30 那筆金額只記了
...
碰了的話 `financial_items` 的 realtime 訂閱會被自己觸發，每 5 秒重載整本私帳（一千多筆交易）
...
- 快車道不把狀態列切成「更新中」；每次成功只安靜更新台股／美股的秒級時間，讓人看得出輪詢仍在運作
- 快車道價格真的讓目前頁面的淨資產改變時，淨資產數字會短暫亮一下；價格相同時不製造假動畫
...
開 App 的首屏**不等待私帳**。資產列的股數與市值是 `sync_klfan_financial_item()` 觸發器算好存在
`financial_items` 的；首屏完成後才用 `requestIdleCallback`（不支援時退回短延遲 timer）在背景預載
92 KB 私帳，讓股票分析一點就能進去：
...
`tests/awake-autorefresh.test.mjs` 守住「首屏不等待私帳、背景最多預載一次、切到個人頁與行情更新
...
所以模型走 `portfolioModelFor(ownerScope)`：用 `portfolioRevision`（私帳重載、報價套用、
