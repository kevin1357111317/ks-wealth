...
- 股票沒有名稱欄，`klfan_stocks.display` 新標的用裸代號、既有標的沿用原本的名字
  （多半是匯入時的中文名，改代號不會把它蓋掉）
...
「股票投資」清單的卡片是**就地展開**的，不跳頁：展開後在原位往下長出淨投入／目前市值長條、
已實現與未實現損益、累計股息、投資期間、首筆交易、目前股價與持有股數，以及完整交易紀錄。
一次只開一檔，再點一次收起來。
