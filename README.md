...
行情抓不到的標的不會假造價格；回應會帶缺漏檔數並顯示在圖表期間旁。但**每日快照是整天全有或全無**：
持股裡只要有一檔當天沒有收盤價，`buildHistoricalSnapshots()` 就不輸出那一天，所以快照序列會出現
空洞。因此 `buildPerformanceSeries()` 扣的現金流是「上一個有快照的日子之後到今天為止」的區間總和，
不是只扣當天的那一筆 —— 只扣當天的話，空洞期間的買進與賣出完全沒被扣掉，賣出價金會整筆被當成漲幅。
實測：抓不到報價的代號買進 50 萬、賣掉時才接回快照，一段只該是 +19% 的期間會被畫成 +71%。
出清到市值 0 的日子也會被跳過（`value <= 0` 就 continue），同一個區間加總順便修掉「全部賣光隔天買回，
出清那天的漲幅整段消失」—— 舊版會把 140 畫成 129.23。兩個案例都在
`tests/portfolio-performance.test.mjs`，把區間加總改回只扣當天就會紅。
