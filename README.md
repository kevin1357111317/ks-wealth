...
固定 50/50。上方摘要的 XIRR 仍是完整交易台帳的實際資金年化報酬，TWR、XIRR 與累計損益率不互換。

Yahoo 的 `close` 已回溯調整拆股、但不調整現金股息；持股市值繼續使用 `close`，台帳股息當提款
...
倍率同時支援小於 1 的反向調整：長榮 2022 現金減資後，Yahoo 會把 2021 歷史價格回溯調高，而台帳
仍保存減資前股數；舊邏輯因此在 2021-11-08 產生約 +37.4% 的假尖峰。現在以 0.4 倍重建舊股數，並
移除原本直接略過極端單日報酬的遮罩，之後異常資料會在測試與圖表中暴露而不是被靜默隱藏。

每日 TWR 扣掉的當日金流，用「當日收盤價 × 成交股數」計價，不用台帳的成交金額。市值本來就是收盤價
...
年化 TWR 從 17.59% 回到 28.70%；XIRR 27.09% 完全不變 —— XIRR 是資金加權，本來就該用台帳的真實現金金額，所以
...
- **股息對齊除息日。** 台帳記的是入帳日，價格卻是在除息日掉的，兩者常差 20~55 天。算在入帳日等於
  除息日先跌一次、入帳日再漲一次；同一期間內大致抵銷，跨期間邊界就會少算或多算。改成用 Yahoo
  `events.dividends` 的除息日（差超過 120 天就不算同一次配息，退回台帳日期）。XIRR 用的
...
- **股數倍率改用真的拆股事件收斂。** 原本純粹用「成交單價 ÷ 當日收盤價」在十五個候選倍率裡猜，
  容忍度 18%。現在若 Yahoo `events.splits` 有資料，候選只留「1（台帳已換算）」與「成交日之後的累計
  倍率（台帳是成交當時股數）」兩個；沒有資料才退回原本的猜測清單。NVDA 2024-06-10 的 10:1 因此
  不再有誤判空間。
...
- **同一段期間的倍率只決定一次。** 台帳的單位在同一個拆股期間只會有一種慣例（要嘛全是成交當時
  股數，要嘛全部已經換算）。逐筆各自判斷會被一筆壞資料帶走：0050 台帳 2023-01-30 那筆金額只記了
